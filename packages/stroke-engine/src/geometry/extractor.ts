...
import { simplifyRDP, getToleranceForLevel } from './simplification';
...
function processCandidatePath(
  ...
  const minPathLength = options?.minPathLength ?? 0.003;
  if (arcLength < minPathLength) {
    return null;
  }

  // 3. Adaptive RDP Simplification
  const tolerance = getToleranceForLevel(level, options?.tolerances);
...
function harvestSubjectCandidates(
  ...
  if (subject.face) {
    ...
    if (noseTip && noseTip.points && noseTip.points.length >= 2 && vis?.nose !== 'occluded' && vis?.nose !== 'not_detected') {
      candidates.push({
        id: `${subjectId}_face_nose_tip`,
        source: 'face_contour',
        region: 'nose',
        level: 2,
        points: noseTip.points,
        closed: false,
    ...
    if (chin && chin.points && chin.points.length >= 2 && vis?.chin !== 'occluded' && vis?.chin !== 'not_detected') {
      candidates.push({
  ...
  // ----------------------------------------------------
  // 4. Body Features & Pose Skeleton
  // ----------------------------------------------------
  if (subject.body) {
    const body = subject.body;

    // Structural limbs/torso
    ...
    // Pose Skeletal Connections
    if (options?.includePoseConnections !== false && body.pose?.connections) {
  ...
  // ----------------------------------------------------
  // 5. Semantic Mask Boundaries
  // ----------------------------------------------------
  if (options?.extractSemanticMaskContours !== false && subject.semanticSegmentation?.masks) {
    ...
    subject.semanticSegmentation.masks.forEach(mask => {
      // Don't extract background mask or unknown
      ...
      if (mask.category === 'hair') {
        region = 'hair';
        level = 3;
      } else if (mask.category === 'face_skin') {
        region = 'face_contour';
        level = 3;
