...
| **TASK-114** | Photo-Exact Likeness, Multi-Modal Reconstruction & Artifact Removal | **COMPLETE** | P0 | TASK-113 | `apps/web/src/vision/mediapipe/*`, `packages/structural-analysis/src/reconstruction/*`, `packages/stroke-engine/*` | Fixed MediaPipe eyelid indices inversion (upper vs lower lid); fixed oral fissure 11-point seam; fixed semantic segmentation array mask retrieval; re-enriched reconstruction with unified face+pose+masks+luminance; replaced balloon cranial dome with natural bob hair flow; eliminated closed clothing polygon chest triangle; smoothed jawline and eyelids | Verified all 23 monorepo test suites pass (325+ tests), browser subagent visual audit confirms photo-exact likeness matching BM-01 with zero visual defects |
