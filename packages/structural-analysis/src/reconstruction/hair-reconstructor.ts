...
export function reconstructHair(
  ...
  // 3. Directional Flow Curves (cascading down left and right flanks OUTSIDE the face)
  // Left flank flow: sweeps from temple/ear down past jaw, curling softly inward
  ...
  // Layered natural pencil hair strands with soft curl
  for (let k = 0; k < 10; k++) {
    const t = k / 9;
    const lX1 = clamp(fBox.x - fBox.width * (0.04 + t * 0.06));
    const lX2 = clamp(fBox.x + fBox.width * (0.01 + t * 0.03));
    strandGroups.push({
      id: `${subjectId}_hair_strand_l_${k}`,
      region: 'hair',
      points: [
        { x: lX1, y: clamp(fBox.y + fBox.height * (0.35 + t * 0.12)) },
        { x: clamp(lX1 - 0.008), y: clamp(fBox.y + fBox.height * (0.65 + t * 0.08)) },
    ...
    const rX1 = clamp(fBox.x + fBox.width * (1.02 + t * 0.07));
    const rX2 = clamp(fBox.x + fBox.width * (0.95 - t * 0.04));
    strandGroups.push({
      id: `${subjectId}_hair_strand_r_${k}`,
      region: 'hair',
      points: [
        { x: rX1, y: clamp(fBox.y + fBox.height * (0.35 + t * 0.12)) },
        { x: clamp(rX1 + 0.008), y: clamp(fBox.y + fBox.height * (0.65 + t * 0.08)) },
